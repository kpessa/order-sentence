...
describe('splPrioritization', () => {
  ...
  describe('performSplPrioritization', () => {
    ...
    const createMockXmlResponse = (
      effectiveTime?: string,
      formCodes?: string[],
      dosageAndAdminText?: string
    ) => ({
      document: {
        ...
        component: {
          manufacturedProduct: {
            manufacturedProduct: {
              formCode: formCodes
                ? formCodes.map((code) => ({ displayName: code }))
                : undefined,
            },
          },
          structuredBody: {
            component: [
              {
                section: {
                  ...
                  title: dosageAndAdminText
                    ? 'Dosage and Administration'
                    : 'Other Section',
                  text: dosageAndAdminText
                    ? { _: dosageAndAdminText }
                    : undefined,
                },
              },
            ],
    ...
    it('should return empty object when no SPL details have XML content', async () => {
      const input = {
        set1: {
          status: 'succeeded',
          data: createMockSplDetail('set1'),
          error: null,
        },
        set2: {
          status: 'succeeded',
          data: createMockSplDetail('set2'),
          error: null,
        },
      };
    ...
    it('should parse single SPL with basic data', async () => {
      const xmlContent =
        '<document><effectiveTime value="20240101"/></document>';
      const input = {
        set1: createMockSplEntry('set1', xmlContent),
      };

      mockParseStringPromise.mockResolvedValue(
        createMockXmlResponse('20240101', ['TABLET'])
      );
    ...
    it('should handle SPLs with no dosage forms (use UNKNOWN_DOSAGE_FORM)', async () => {
      ...
      mockParseStringPromise.mockResolvedValue(
        createMockXmlResponse('20240101', [])
      );
    ...
    it('should handle multiple dosage forms (use first one as key)', async () => {
      ...
      mockParseStringPromise.mockResolvedValue(
        createMockXmlResponse('20240101', ['TABLET', 'CAPSULE'])
      );
    ...
    it('should extract dosage and administration text', async () => {
      ...
      mockParseStringPromise.mockResolvedValue(
        createMockXmlResponse('20240101', ['TABLET'], dosageText)
      );
      ...
      mockParseStringPromise
        .mockResolvedValueOnce({
          document: {
            ...
            component: {
              manufacturedProduct: {
                manufacturedProduct: { formCode: { displayName: 'TABLET' } },
              },
            },
          },
        })
        .mockResolvedValueOnce({
          document: {
            ...
            component: {
              manufacturedProduct: {
                manufacturedProduct: { formCode: { displayName: 'CAPSULE' } },
              },
            },
          },
        })
        .mockResolvedValueOnce({
          document: {
            ...
            component: {
              manufacturedProduct: {
                manufacturedProduct: { formCode: { displayName: 'SYRUP' } },
              },
            },
          },
        });
