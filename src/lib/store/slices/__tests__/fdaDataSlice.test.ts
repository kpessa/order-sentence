...
describe('fdaDataSlice', () => {
  ...
  describe('reducers', () => {
    describe('resetFdaState', () => {
      it('should reset state to initial values', () => {
        ...
        store.dispatch(fetchNdcsByRxcui.fulfilled(mockNdcList, 'requestId', mockRxcui))
  ...
  describe('selectors', () => {
    it('should select FDA data state', () => {
      const state = selectFdaDataState(store.getState())
      expect(state).toEqual(store.getState().fdaData)
    })

    it('should select NDC list', () => {
      const ndcList = selectNdcList(store.getState())
      expect(ndcList).toEqual([])
    })

    it('should select OpenFDA results', () => {
      const results = selectOpenFdaResults(store.getState())
      expect(results).toEqual([])
    })

    it('should select FDA data status', () => {
      const status = selectFdaDataStatus(store.getState())
      expect(status).toBe('idle')
    })

    it('should select FDA data error', () => {
      const error = selectFdaDataError(store.getState())
      expect(error).toBe(null)
    })

    it('should select current RXCUI', () => {
      const rxcui = selectCurrentRxcui(store.getState())
      expect(rxcui).toBe(undefined)
    })
  })
  ...
  describe('fetchNdcsByRxcui async thunk', () => {
    it('should successfully fetch NDCs by RXCUI', async () => {
      ...
      const result = await store.dispatch(fetchNdcsByRxcui(mockRxcui))
    ...
    it('should handle non-200 responses', async () => {
      ...
      expect(result.type).toBe('fdaData/fetchNdcsByRxcui/rejected')
      expect(result.payload).toBe('RxNorm API Error: 500 Internal Server Error')
    })
  })
  ...
  describe('fetchFdaDataByNdcs async thunk', () => {
    it('should successfully fetch FDA data by NDCs', async () => {
      ...
      const result = await store.dispatch(fetchFdaDataByNdcs(mockNdcList))
      ...
      expect(state.totalOpenFdaResults).toBe(mockOpenFdaApiResponse.meta.results.total)
    })

    it('should handle empty NDC list', async () => {
      const result = await store.dispatch(fetchFdaDataByNdcs([]))
    ...
    it('should handle 404 responses', async () => {
      ...
      expect(result.type).toBe('fdaData/fetchFdaDataByNdcs/rejected')
      expect(result.payload).toBe('No FDA data found for product NDC: 0045-0005 (derived from 0045-0005-05)')
    })
    ...
    it('should handle response with no results', async () => {
      ...
      expect(result.type).toBe('fdaData/fetchFdaDataByNdcs/rejected')
      expect(result.payload).toBe('No FDA data found in \'results\' for product NDC: 0045-0005')
    })
  })
  ...
  describe('fetchOpenFdaDataByDrugName async thunk', () => {
    it('should successfully fetch FDA data by drug name', async () => {
      ...
      const result = await store.dispatch(fetchOpenFdaDataByDrugName('Lisinopril'))

      expect(result.type).toBe('fdaData/fetchOpenFdaDataByDrugName/fulfilled')
      expect(result.payload.results).toHaveLength(1)
      expect(result.payload.results[0].brand_name).toBe('Lisinopril Brand')
      expect(result.payload.results[0].openfda.application_type).toEqual(['NDA'])
    ...
    it('should handle empty drug name', async () => {
      const result = await store.dispatch(fetchOpenFdaDataByDrugName(''))
    ...
    it('should handle 404 responses', async () => {
      ...
      const result = await store.dispatch(fetchOpenFdaDataByDrugName('NonexistentDrug'))

      expect(result.type).toBe('fdaData/fetchOpenFdaDataByDrugName/rejected')
      expect(result.payload).toBe('No Drugs@FDA data found for drug name: NonexistentDrug. Raw response: Not found')
    })
    ...
    it('should handle response with no results', async () => {
      ...
      expect(result.type).toBe('fdaData/fetchOpenFdaDataByDrugName/rejected')
      expect(result.payload).toContain('No results in Drugs@FDA data for drug name: NonexistentDrug')
    })
    ...
    it('should filter out non-NDA/ANDA/BLA applications', async () => {
      ...
      const result = await store.dispatch(fetchOpenFdaDataByDrugName('Test'))
  ...
  describe('fetchSplDetailFromDailyMed async thunk', () => {
    it('should successfully fetch SPL detail from DailyMed', async () => {
      ...
      const result = await store.dispatch(fetchSplDetailFromDailyMed(mockSetId))
  ...
  describe('fetchSplsFromDailyMedByName async thunk', () => {
    it('should successfully fetch SPLs from DailyMed by name', async () => {
      ...
      const result = await store.dispatch(fetchSplsFromDailyMedByName('Lisinopril'))
    ...
    it('should handle empty drug name', async () => {
      const result = await store.dispatch(fetchSplsFromDailyMedByName(''))

      expect(result.type).toBe('fdaData/fetchSplsFromDailyMedByName/rejected')
      expect(result.payload).toBe('No drug name provided to fetch SPLs from DailyMed.')
    })
    ...
    it('should handle 404 responses', async () => {
      ...
      const result = await store.dispatch(fetchSplsFromDailyMedByName('NonexistentDrug'))

      expect(result.type).toBe('fdaData/fetchSplsFromDailyMedByName/rejected')
      expect(result.payload).toBe('No DailyMed SPLs found for drug name: NonexistentDrug. Response: Not found')
    })
    ...
    it('should handle response with unexpected structure', async () => {
      ...
      const result = await store.dispatch(fetchSplsFromDailyMedByName('TestDrug'))
  ...
  describe('extraReducers', () => {
    it('should handle fetchNdcsByRxcui.pending', () => {
      ...
      const state = fdaDataReducer(undefined, pendingAction)

      expect(state.status).toBe('loading')
      expect(state.error).toBe(null)
      expect(state.currentEndpoint).toBe(`RxNorm /rxcui/${mockRxcui}/ndcs.json`)
      expect(state.rxcui).toBe(mockRxcui)
    ...
    it('should handle fetchOpenFdaDataByDrugName.pending', () => {
      ...
      expect(state.status).toBe('loading')
      expect(state.error).toBe(null)
      expect(state.currentEndpoint).toBe(`Drugs@FDA /drugsfda.json with drugName: ${drugName}`)
      expect(state.openFdaResults).toEqual([])
      expect(state.totalOpenFdaResults).toBe(0)
