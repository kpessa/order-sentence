...
import {
  selectFdaDataState,
  OpenFdaResult,
  fetchSplsFromDailyMedByName, // New thunk
...
import { performSplPrioritization, ParsedSplProductData } from '@/lib/utils/splPrioritization'; // Added ParsedSplProductData import
...
export function OpenFdaResultsDisplay() {
  ...
  const [expandedDosageForms, setExpandedDosageForms] = useState<Record<string, boolean>>({});
    ...
    if (openFdaResults.length > 0 && Object.keys(dailyMedDetails).length > 0) {
      ...
      openFdaResults.forEach(result => {
        if (result.openfda?.spl_set_id) {
          result.openfda.spl_set_id.forEach(setId => allSetIdsFromOpenFda.add(setId));
      ...
      if (allDetailsAttempted) {
        console.log("[OpenFdaResultsDisplay useEffect dailyMedDetails] All targeted SPL details have at least an entry. Proceeding to check for XML content.");
        const successfullyFetchedSplDetails: Record<string, { data?: DailyMedSplDetail; status: string; error?: string | null }> = {};
        let allFetchesDone = true; // Assume all are done initially

        for (const setId of Array.from(allSetIdsFromOpenFda)) {
          const detailEntry = dailyMedDetails[setId];
          if (detailEntry) { // Should always be true due to allDetailsAttempted check
            if (detailEntry.status === 'loading') {
              allFetchesDone = false; // If any are still loading, wait.
              console.log(`[OpenFdaResultsDisplay useEffect dailyMedDetails] Waiting for SPL detail fetch for SETID: ${setId}`);
              break;
            }
            if (detailEntry.status === 'succeeded' && detailEntry.data?.xml_content) {
              successfullyFetchedSplDetails[setId] = detailEntry;
            } else if (detailEntry.status === 'succeeded' && !detailEntry.data?.xml_content) {
              console.warn(`[OpenFdaResultsDisplay useEffect dailyMedDetails] SETID ${setId} fetched successfully but has no XML content.`);
              // Still include it so prioritization knows it was a valid attempt but had no XML
              successfullyFetchedSplDetails[setId] = detailEntry; 
            } else if (detailEntry.status === 'failed') {
              console.error(`[OpenFdaResultsDisplay useEffect dailyMedDetails] Failed to fetch SPL detail for SETID ${setId}: ${detailEntry.error}`);
              successfullyFetchedSplDetails[setId] = detailEntry; // Include failed attempts too
            }
          } 
        }

        if (allFetchesDone && Object.keys(successfullyFetchedSplDetails).length > 0) {
          console.log("[OpenFdaResultsDisplay useEffect dailyMedDetails] All XML fetches complete. Performing prioritization on (summarized input count):", Object.keys(successfullyFetchedSplDetails).length);
          console.log("[OpenFdaResultsDisplay useEffect dailyMedDetails] Successfully fetched SPL details for prioritization (keys):", Object.keys(successfullyFetchedSplDetails));
          ...
          console.log("[OpenFdaResultsDisplay useEffect dailyMedDetails] Data for performSplPrioritization (summary):", summaryForLog);

          performSplPrioritization(successfullyFetchedSplDetails)
            .then(prioritized => {
              dispatch(setPrioritizedSpls(prioritized));
              console.log("[OpenFdaResultsDisplay useEffect dailyMedDetails] Dispatched setPrioritizedSpls with (summarized output count):", Object.keys(prioritized).length);
            })
            .catch(error => {
              console.error("[OpenFdaResultsDisplay useEffect dailyMedDetails] Error during performSplPrioritization:", error);
              // Optionally dispatch an error state for prioritization itself
            });
        } else if (allFetchesDone) {
          console.log("[OpenFdaResultsDisplay useEffect dailyMedDetails] All XML fetches done, but no SPLs with XML content were successfully fetched to prioritize.");
          dispatch(setPrioritizedSpls({})); // Clear any previous prioritized SPLs
        }
      } else {
        console.log("[OpenFdaResultsDisplay useEffect dailyMedDetails] Not all targeted SPL details have an entry yet. Waiting...");
      }
    }
  ...
  useEffect(() => {
    if (prioritizedSpls && Object.keys(prioritizedSpls).length > 0) {
      console.log("[OpenFdaResultsDisplay] Prioritized SPLs updated (summarized with D&A info):", 
        Object.fromEntries(
          Object.entries(prioritizedSpls).map(([key, spl]) => [
            key,
            {
              spl_set_id: spl.spl_set_id,
              xml_content_summary: spl.xml_content ? `XML (length: ${spl.xml_content.length})` : 'No XML Content',
              dosageAndAdministrationFound: spl.dosageAndAdministrationText ? `Yes (length: ${spl.dosageAndAdministrationText.length})` : 'No',
            }
          ])
        )
      );
      ...
      openFdaResults.forEach(result => {
        if (result.openfda?.spl_set_id) {
          result.openfda.spl_set_id.forEach(setId => openFdaResultsMap.set(setId, result));
        } else if (result.set_id) { 
      ...
      Object.entries(prioritizedSpls).forEach(([dosageForm, spl]) => {
        if (spl.dosageAndAdministrationText) {
          ...
          const brandName = associatedFdaResult?.openfda?.brand_name?.join(', ') || associatedFdaResult?.brand_name || 'Unknown Brand';
          const genericName = associatedFdaResult?.openfda?.generic_name?.join(', ') || associatedFdaResult?.generic_name || 'Unknown Generic';
      ...
      console.log(`[OpenFdaResultsDisplay] Dosage & Administration Text found in ${dnaFoundCount} prioritized SPL(s).`);
      if (dnaFoundCount > 0) {
        console.log("[OpenFdaResultsDisplay] Compiled Dosage & Administration Text (with drug names & forms):\n", compiledDnaText);
  ...
  useEffect(() => {
    ...
    console.log("[OpenFdaResultsDisplay RENDER] prioritizedSpls VALUE (summarized with D&A):", 
    ...
    console.log("[OpenFdaResultsDisplay RENDER] dailyMedDetails count:", Object.keys(dailyMedDetails).length);

    // Log SPL counts
    const initialFetchCount = Object.keys(dailyMedDetails).length;
    const successfullyParsedCount = Object.values(dailyMedDetails).filter(d => d.status === 'succeeded' && d.data?.xml_content).length;
    ...
    const prioritizedCount = prioritizedSpls ? Object.keys(prioritizedSpls).length : 0;
    let prioritizedWithDnaCount = 0;
    if (prioritizedSpls) {
        prioritizedWithDnaCount = Object.values(prioritizedSpls).filter(p => p.dosageAndAdministrationText).length;
    }

    console.log(`[OpenFdaResultsDisplay RENDER SPL Counts] Initial SETIDs for potential prioritization: ${initialFetchCount}, Successfully fetched with XML: ${successfullyParsedCount}, Prioritized SPLs: ${prioritizedCount}, Prioritized with D&A Text: ${prioritizedWithDnaCount}`);
  ...
  if (openFdaStatus === 'loading') {
    return (
      <div className="mt-6 p-4 border rounded-lg shadow bg-gray-50">
        <h3 className="text-lg font-semibold text-gray-700 mb-2">Fetching OpenFDA Approved Products...</h3>
        <p className="text-sm text-gray-600">Querying for: {currentDrugNameQuery || currentEndpoint || 'N/A'}</p>
        {/* Basic pulse animation placeholder */}
        <div className="animate-pulse mt-3 flex space-x-4">
          <div className="flex-1 space-y-6 py-1">
            <div className="h-2 bg-slate-200 rounded"></div>
            <div className="space-y-3"><div className="h-2 bg-slate-200 rounded"></div></div>
          </div>
        </div>
      </div>
    );
  ...
  if (openFdaStatus === 'failed') {
    return (
      <div className="mt-6 p-4 border border-red-300 rounded-lg shadow bg-red-50">
        <h3 className="text-lg font-semibold text-red-700 mb-2">Error Fetching OpenFDA Data</h3>
        <p className="text-sm text-red-600">Details: {openFdaError || 'Unknown error'}</p>
        <p className="text-xs text-gray-500 mt-1">Queried: {currentDrugNameQuery || currentEndpoint || 'N/A'}</p>
      </div>
    );
  }
  ...
  if (openFdaStatus === 'fda_queried') {
    if (openFdaResults.length === 0) {
      return (
        <div className="mt-6 p-4 border rounded-lg shadow bg-yellow-50">
          <h3 className="text-lg font-semibold text-yellow-700 mb-2">No Approved Products Found on OpenFDA</h3>
          <p className="text-sm text-yellow-600">No products matching the criteria (NDA, ANDA, BLA) were found for '{currentDrugNameQuery}'.</p>
        </div>
      );
    }

    const shouldShowPrioritizedSpls = prioritizedSpls && Object.keys(prioritizedSpls).length > 0;
    ...
    const handlePrioritizeSpls = async () => {
      ...
      console.log("[OpenFdaResultsDisplay handlePrioritizeSpls] Initiating SPL XML fetching.");
      ...
      if (openFdaResults && openFdaResults.length > 0) {
        openFdaResults.forEach(result => {
          if (result.openfda?.spl_set_id) {
            result.openfda.spl_set_id.forEach(setId => {
              ...
              if (!dailyMedDetails[setId] || (dailyMedDetails[setId]?.status !== 'succeeded' && dailyMedDetails[setId]?.status !== 'loading')) {
                setIdsToFetch.add(setId);
              }
            });
          } else if (result.set_id) { // Fallback for results directly having set_id
              if (!dailyMedDetails[result.set_id] || (dailyMedDetails[result.set_id]?.status !== 'succeeded' && dailyMedDetails[result.set_id]?.status !== 'loading')) {
                setIdsToFetch.add(result.set_id);
              }
          }
        });

        if (setIdsToFetch.size === 0 && Object.keys(dailyMedDetails).length > 0) {
          console.log("[OpenFdaResultsDisplay handlePrioritizeSpls] All necessary SPL XMLs seem to be already fetched or loading. Prioritization should occur via useEffect if data is valid.");
        ...
        console.log(`[OpenFdaResultsDisplay handlePrioritizeSpls] Identified ${setIdsToFetch.size} unique SET IDs to fetch XML for.`);
        setIdsToFetch.forEach(setId => {
          console.log(`[OpenFdaResultsDisplay handlePrioritizeSpls] Dispatching fetchSplDetailFromDailyMed for SETID: ${setId}`);
          dispatch(fetchSplDetailFromDailyMed(setId));
        });

      } else {
        console.warn("[OpenFdaResultsDisplay handlePrioritizeSpls] No OpenFDA results available to determine which SPLs to fetch and prioritize.");
      }
    };
      ...
      <div className="mt-6 p-4 border rounded-lg shadow bg-gray-50">
        <div className="flex justify-between items-center mb-4">
            ...
            <button 
                ...
                disabled={dailyMedSplListStatus === 'loading' || Object.keys(dailyMedDetails).length === 0}
                ...
                {dailyMedSplListStatus === 'loading' ? 'Fetching SPLs...' : 'Prioritize SPLs'}
            </button>
        </div>
        ...
        {retrievalTimestamp && (
            <p className="text-xs text-gray-500 mb-3">
                Data fetched from OpenFDA on: {new Date(retrievalTimestamp).toLocaleString()}
            </p>
        )}
        ...
        {shouldShowPrioritizedSpls ? (
          <section className="my-6 p-4 border rounded-lg bg-green-50">
            <h3 className="text-xl font-semibold text-green-700 mb-3">Prioritized DailyMed Labels (XML Fetched)</h3>
            {Object.entries(prioritizedSpls).map(([dosageForm, splDetail]) => (
              <div key={dosageForm} className="mb-4 p-3 border rounded-md bg-white shadow">
                <h4 className="text-md font-semibold text-gray-800">{dosageForm} (Note: Prioritization logic needs update for XML parsing)</h4>
                {splDetail && splDetail.spl_set_id ? (
                  <>
                    <p className="text-sm text-gray-700"><strong>SET ID:</strong> {splDetail.spl_set_id}</p>
                    {splDetail.xml_content ? (
                      <p className="text-sm text-gray-700"><strong>XML Content Fetched:</strong> Yes (Length: {splDetail.xml_content.length})</p>
                    ) : (
                      <p className="text-sm text-red-500"><strong>XML Content Fetched:</strong> No</p>
                    )}
                    <a 
                  ...
                  <p className="text-sm text-gray-500">No SPL data available or prioritized for this dosage form.</p>
                )}
              </div>
            ))}
          </section>
        ) : (
          <p className="text-sm text-gray-600">No prioritized SPLs to display. Click "Prioritize SPLs" after fetching data.</p>
        )}
            ...
            <div key={dosageForm} className="mb-8">
              <div className="flex justify-between items-center mb-3 sticky top-0 bg-white py-2 z-10 border-b">
                ...
                {resultsInGroup.length > 0 && ( // Show button if there are items
                  ...
                  >
                    {isExpanded ? 'Collapse' : `Show (${resultsInGroup.length} items)`}
                  </button>
                )}
              </div>
              {isExpanded && resultsInGroup.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {resultsInGroup.slice(0, itemsToShow).map((result, index) => ( // slice is still fine, itemsToShow is now 0 or full length
                    <div 
                      key={result.id || `openfda-result-${dosageForm}-${index}`} 
                    ...
                    >
                      <div>
                        <h4 className="text-md font-semibold text-gray-800 mb-1">
                          {result.brand_name || result.openfda?.brand_name?.[0] || 'N/A'} 
                          {result.dosage_form ? <span className="text-xs text-gray-500"> ({result.dosage_form})</span> : ''}
                        </h4>
                        <p className="text-xs text-gray-600 mb-0.5">
                          <strong>Generic:</strong> {result.generic_name || result.openfda?.generic_name?.[0] || 'N/A'}
                        </p>
                        {/* Display dosage_form and product_type more explicitly */}
                        {result.dosage_form && (
                          <p className="text-xs text-gray-600 mb-0.5">
                            <strong>Dosage Form (transformed):</strong> {result.dosage_form}
                          </p>
                        )}
                        {result.product_type && (
                          <p className="text-xs text-gray-600 mb-0.5">
                            <strong>Product Type (root):</strong> {result.product_type}
                          </p>
                        )}
                         {result.openfda?.product_type && result.openfda.product_type.length > 0 && (
                          <p className="text-xs text-gray-600 mb-0.5">
                            <strong>Product Type (openfda):</strong> {result.openfda.product_type.join(', ')}
                          </p>
                        )}
                        <p className="text-xs text-gray-600 mb-0.5">
                          <strong>Manufacturer:</strong> {result.openfda?.manufacturer_name?.join(', ') || 'N/A'}
                        </p>
                        <p className="text-xs text-gray-600">
                          <strong>Application No:</strong> {result.application_number || 'N/A'} 
                          {result.openfda?.application_type ? ` (${result.openfda.application_type.join(', ')})` : ''}
                        </p>
                        {result.openfda?.spl_set_id && result.openfda.spl_set_id.length > 0 && (
                          <p className="text-xs text-gray-600 mt-1">
                            <strong>SPL Set ID(s):</strong> {result.openfda.spl_set_id.join(', ')}
                          </p>
                        )}
                      </div>
                      ...
                      {result.openfda?.spl_set_id && result.openfda.spl_set_id.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-gray-200">
                          {result.openfda.spl_set_id.map(setId => {
                            ...
                            const isFetchFailed = detailEntry?.status === 'failed';
                            const fetchedDetail = detailEntry?.status === 'succeeded' ? detailEntry.data : null;

                            return (
                              <div key={setId} className="mb-2 last:mb-0">
                                <button 
                                  onClick={() => dispatch(fetchSplDetailFromDailyMed(setId))}
                                  disabled={isLoadingDetail || !!fetchedDetail}
                                  className={`text-xs px-2 py-1 rounded shadow-sm transition-colors 
                                    ${isLoadingDetail ? 'bg-yellow-400 cursor-wait' : 
                                    fetchedDetail ? 'bg-green-200 text-green-800 cursor-not-allowed' : 
                                    isFetchFailed ? 'bg-red-400 hover:bg-red-500 text-white' : 
                                    'bg-blue-500 hover:bg-blue-600 text-white'}`}
                                >
                                  {isLoadingDetail ? 'Fetching...' : 
                                   fetchedDetail ? `Fetched: ${setId.substring(0,8)}...` : 
                                   isFetchFailed ? `Retry: ${setId.substring(0,8)}...` : 
                                   `Fetch DailyMed: ${setId.substring(0,8)}...`}
                                </button>
                                {isFetchFailed && detailEntry?.error && (() => {
                                  const errorValue = detailEntry.error;
                                  let displayMessage = 'Unknown error';
                                  if (typeof errorValue === 'string') {
                                    displayMessage = errorValue;
                                  } else if (errorValue && typeof errorValue === 'object' && 'error' in errorValue && typeof (errorValue as any).error === 'string') {
                                    // Specifically handles if errorValue is { splSetId: string, error: string }
                                    displayMessage = (errorValue as any).error;
                                  } else if (errorValue && typeof errorValue === 'object' && 'message' in errorValue && typeof (errorValue as any).message === 'string') {
                                    // Handles if errorValue is like a SerializedError { message: string }
                                    displayMessage = (errorValue as any).message;
                                  }
                                  return <p className="text-xs text-red-500 mt-1">Error: {displayMessage}</p>;
                                })()}
                                {fetchedDetail && (
                                  <div className="mt-1 p-2 bg-gray-100 rounded text-xs">
                                    <p><strong>SET ID:</strong> {fetchedDetail.spl_set_id}</p>
                                    {fetchedDetail.xml_content ? (
                                      <p><strong>XML Available:</strong> Yes (Length: {fetchedDetail.xml_content.length})</p>
                                    ) : (
                                      <p><strong>XML Available:</strong> No</p>
                                    )}
                                    <a 
        ...
        {(Object.keys(dailyMedDetails).length > 0) && prioritizedSpls && ( // Show if any fetches were attempted
          ...
          <div id="prioritized-spls-section" className="mt-6 pt-4 border-t">
            <h3 className="text-xl font-semibold text-green-700 mb-3">Prioritized Package Insert per Dosage Form:</h3>
            {shouldShowPrioritizedSpls ? (
              ...
              <p className="text-sm text-gray-500">
                {Object.values(dailyMedDetails).some(d => d.status === 'succeeded' && d.data) 
        ...
        {showDnaPreviewModal && compiledDnaForPreview && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-3xl w-full max-h-[80vh] flex flex-col">
              <h4 className="text-xl font-semibold mb-4">Compiled Dosage & Administration Text</h4>
              <div className="overflow-y-auto flex-grow mb-4">
                <pre className="text-xs whitespace-pre-wrap bg-gray-50 p-3 rounded">{compiledDnaForPreview}</pre>
              </div>
              <button 
  ...
  return (
    <div className="mt-6 p-4">
      <p className="text-sm text-gray-500">Enter a drug name to begin the search.</p>
    </div>
  );
} 