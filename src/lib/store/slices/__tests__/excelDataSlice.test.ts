...
describe('excelDataSlice', () => {
  ...
  beforeEach(() => {
    ...
    mockSheetToJson = XLSX.utils.sheet_to_json as jest.MockedFunction<
  ...
  describe('selectors', () => {
    ...
    it('should select excel data error', () => {
      const state = store.getState();
      expect(state.excelData.error).toBe(null);
    });
  });
  ...
  describe('fetchExcelData async thunk', () => {
    ...
    it('should handle insufficient rows for header', async () => {
      const insufficientRows = [['Only one row']];
      ...
      expect(result.payload).toBe(
        'Header row 2 is out of bounds. Sheet has 1 rows.'
      );
    });
  ...
  describe('integration with store', () => {
    it('should work with selectors after failed fetch', async () => {
      ...
      const state = store.getState();
      expect(state.excelData.data).toEqual([]);
      expect(state.excelData.loading).toBe('failed');
      expect(state.excelData.error).toBe('Network error');
    });
  });
});
