...
export function ColumnHeaderFilterPopover<TData, TValue>({
  ...
  const currentColumnFilterValue = column.getFilterValue() as
    | string
    | string[]
    | undefined;
  ...
  const handlePillClick = (value?: string, event?: React.MouseEvent) => {
    ...
    if (value === undefined) {
      ...
      if (Array.isArray(currentColumnFilterValue)) {
        ...
        typeof currentColumnFilterValue === 'string' &&
        currentColumnFilterValue !== ''
      ) {
  ...
  const isActiveFilter =
    currentColumnFilterValue !== undefined &&
    ((typeof currentColumnFilterValue === 'string' &&
      currentColumnFilterValue !== '') ||
      (Array.isArray(currentColumnFilterValue) &&
        currentColumnFilterValue.length > 0));
  ...
  const isPillActive = (pillLabel: string) => {
    ...
    return (
      typeof currentColumnFilterValue === 'string' &&
      currentColumnFilterValue === pillLabel
    );
  };

  const isAllPillActive =
    currentColumnFilterValue === undefined ||
    (Array.isArray(currentColumnFilterValue) &&
      currentColumnFilterValue.length === 0);
          ...
          <div className="flex justify-between items-center">
            ...
            {isActiveFilter && (
              <Button
                ...
                size="xs"
                onClick={clearFilter}
                className="h-6 p-1 text-xs text-red-500 hover:text-red-600"
              >
            ...
            <div className="pt-2 pb-1 space-y-1 border-b border-gray-200 dark:border-gray-700 mb-2">
              ...
              <div className="flex gap-1.5">
                ...
                <PillButton
                  ...
                  onClick={() =>
                    onDescriptionCombinationFilterChange('combination')
                  }
                  columnName={columnNameForPillStyling} // Or a specific style
                  className="flex-1 justify-center text-xs"
                />
          ...
          {uniqueValuesForPills && uniqueValuesForPills.length > 0 && (
            <div className="space-y-1 pt-2">
              <p className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                Quick select (Ctrl+Click for multi):
              </p>
              <PillButton
                label="All"
              ...
              {uniqueValuesForPills.length > 10 && (
                <p className="text-xs text-gray-400 dark:text-gray-500 text-center mt-1">
                  +{uniqueValuesForPills.length - 10} more
                </p>
              )}
            </div>
          )}
          {(!uniqueValuesForPills || uniqueValuesForPills.length === 0) &&
            ...
            currentColumnFilterValue === undefined && (
              <p className="text-xs text-gray-400 dark:text-gray-500 pt-1">
                No quick selects. Type to search.
              </p>
            )}
        </div>
      </PopoverContent>
