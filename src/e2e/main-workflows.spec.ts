...
test.describe('Main Application Workflows', () => {
  ...
  test.describe('Drug Search Workflow', () => {
    test('should allow users to search for a drug and see results', async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="drug-suggestion"]').first()
      ...
      await expect(page.locator('#selected-drug-info')).toContainText(
  ...
  test.describe('Workflow Selection', () => {
    ...
    test('should enable workflow options after drug selection', async ({
      page,
    }) => {
      const workflowSection = page.locator('#workflow-selection');
      await expect(workflowSection).not.toHaveClass(/opacity-50/);
      ...
      const buttonCount = await workflowSection.locator('button').count();
    ...
    test('should navigate to Excel viewer workflow', async ({ page }) => {
      ...
      const excelViewerButton = workflowSection.locator(
        'button:has-text("Excel Viewer"), button:has-text("View Orders"), button:has-text("Order Sentences")'
      );
    ...
    test('should trigger FDA data workflow', async ({ page }) => {
      ...
      const fdaButton = workflowSection.locator(
        'button:has-text("FDA Data"), button:has-text("Get FDA"), button:has-text("OpenFDA")'
      );
      ...
      if ((await fdaButton.count()) > 0) {
        ...
        await expect(
          page.locator('[data-testid="openfda-results"]')
  ...
  test.describe('OpenFDA Results Display', () => {
    ...
    test('should display FDA results with dosage forms', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="openfda-results"]')).toBeVisible(
        { timeout: 15000 }
      );

      // Check for dosage form sections
      const dosageFormSections = page.locator(
        '[data-testid="dosage-form-section"]'
      );
      const sectionCount = await dosageFormSections.count();
    ...
    test('should allow expanding dosage form details', async ({ page }) => {
      ...
      const firstAccordion = page
        .locator('[data-testid="dosage-form-accordion"]')
        .first();
      ...
      if ((await firstAccordion.count()) > 0) {
        ...
        await expect(
          page.locator('[data-testid="dosage-form-content"]').first()
        ).toBeVisible();
      }
    });
