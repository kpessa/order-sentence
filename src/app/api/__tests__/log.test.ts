...
describe('/api/log API Route', () => {
  ...
  beforeEach(() => {
    ...
    >;
    mockFsExistsSync = fs.existsSync as jest.MockedFunction<
      typeof fs.existsSync
    >;
    mockFsMkdir = fs.promises.mkdir as jest.MockedFunction<
      typeof fs.promises.mkdir
    >;
    mockFsAppendFile = fs.promises.appendFile as jest.MockedFunction<
      typeof fs.promises.appendFile
    >;
    ...
    it('should successfully process valid log entries', async () => {
      ...
      expect(mockFsAppendFile).toHaveBeenCalledWith(
        expect.stringMatching(
          /\/mock\/cwd\/logs\/\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.log$/
        ),
    ...
    it('should handle logs with optional parameters', async () => {
      ...
      expect(mockFsAppendFile).toHaveBeenCalledWith(
        ...
        expect.stringContaining(
          '[INFO] Log with params param1 {"key":"value"} 123'
        )
      );
    });
  ...
  describe('directory creation', () => {
    it('should create log directory if it does not exist', async () => {
      ...
      expect(mockFsMkdir).toHaveBeenCalledWith('/mock/cwd/logs', {
        recursive: true,
      });
    });
