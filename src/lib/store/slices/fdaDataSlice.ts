...
interface RxNormNdcResponse {
  ndcGroup: {
    ndcList?: { // ndcList might be missing if no NDCs are found
      ndc: string[];
    };
  };
}
...
export interface OpenFdaResult {
  ...
  openfda?: { // This structure is common and often populated by openFDA harmonization
...
interface FdaDataState {
  ...
  status: 'idle' | 'loading' | 'succeeded' | 'failed' | 'ndc_resolved' | 'fda_queried';
  error: string | null | undefined;
  currentEndpoint: string | null;
  retrievalTimestamp?: string;

  // New state for DailyMed SPL details
  dailyMedDetails: Record<string, { data?: DailyMedSplDetail; status: 'idle' | 'loading' | 'succeeded' | 'failed'; error?: string | null }>;
...
>(
  ...
  async (ndcList, { rejectWithValue, getState }) => {
    ...
    const productNdc = firstNdc.includes('-') ? firstNdc.substring(0, firstNdc.lastIndexOf('-')) : firstNdc;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
            return rejectWithValue(`No FDA data found for product NDC: ${productNdc} (derived from ${firstNdc})`);
        }
      ...
      return rejectWithValue(`No FDA data found in 'results' for product NDC: ${productNdc}`); 
    } catch (error: any) {
      return rejectWithValue(error.message || 'Failed to fetch data from openFDA');
    }
  }
);
...
>(
  ...
  async (drugName, { rejectWithValue }) => {
    if (!drugName || drugName.trim() === '') {
      console.error('[fetchOpenFdaDataByDrugName] Error: No drug name provided.');
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`[fetchOpenFdaDataByDrugName] Drugs@FDA API response not OK. Status: ${response.status}, Response text: ${responseText}`);
        if (response.status === 404) {
          return rejectWithValue(`No Drugs@FDA data found for drug name: ${drugName}. Raw response: ${responseText}`);
        }
      ...
      console.log('[fetchOpenFdaDataByDrugName] Parsed Drugs@FDA data:', rawDrugsfdaData);
      ...
      if (rawDrugsfdaData.results && rawDrugsfdaData.results.length > 0) {
        ...
        const transformedResults: OpenFdaResult[] = rawDrugsfdaData.results.flatMap((app: any) => {
          const appTypeFromAppOpenFda = app.openfda?.application_type?.[0] || '';
          let baseAppType = '';
          if (app.application_number) {
            if (app.application_number.startsWith('NDA')) baseAppType = 'NDA';
            else if (app.application_number.startsWith('ANDA')) baseAppType = 'ANDA';
            else if (app.application_number.startsWith('BLA')) baseAppType = 'BLA';
          }
          ...
          if (!app.products || app.products.length === 0) {
            ...
            const appSplSetIds = app.openfda?.spl_set_id 
              ? (Array.isArray(app.openfda.spl_set_id) ? app.openfda.spl_set_id : [app.openfda.spl_set_id]) 
              : undefined;

            if (!appSplSetIds || appSplSetIds.length === 0) {
              console.warn(`[fetchOpenFdaDataByDrugName] Application ${app.application_number} has no products and no application-level spl_set_id. Skipping.`);
            ...
            return [{
              ...
              openfda: {
                ...app.openfda,
                application_number: app.application_number ? [app.application_number] : undefined,
                application_type: applicationType ? [applicationType] : undefined,
                spl_set_id: appSplSetIds,
                manufacturer_name: app.sponsor_name ? [app.sponsor_name] : (app.openfda?.manufacturer_name || undefined),
              },
          ...
          return app.products.map((product: any) => {
            // Prioritize product-level SPL Set ID, fallback to application-level
            let productSplSetIds = product.openfda?.spl_set_id 
              ? (Array.isArray(product.openfda.spl_set_id) ? product.openfda.spl_set_id : [product.openfda.spl_set_id])
              : undefined;
            
            if (!productSplSetIds && app.openfda?.spl_set_id) {
                // Fallback to application-level SPL Set IDs if product has none
                productSplSetIds = Array.isArray(app.openfda.spl_set_id) ? app.openfda.spl_set_id : [app.openfda.spl_set_id];
            }
            ...
            if (!productSplSetIds || productSplSetIds.length === 0) {
              console.warn(`[fetchOpenFdaDataByDrugName] Product ${product.product_number} in App ${app.application_number} has no spl_set_id. Skipping product.`);
            ...
            return {
              ...
              application_number: app.application_number,
              brand_name: product.brand_name || app.openfda?.brand_name?.join(', '),
              generic_name: app.openfda?.generic_name?.join(', ') || product.active_ingredients?.[0]?.name,
              openfda: {
                ...
                product_ndc: product.openfda?.product_ndc ? (Array.isArray(product.openfda.product_ndc) ? product.openfda.product_ndc : [product.openfda.product_ndc]) : (app.openfda?.product_ndc || undefined),
                package_ndc: product.openfda?.package_ndc ? (Array.isArray(product.openfda.package_ndc) ? product.openfda.package_ndc : [product.openfda.package_ndc]) : (app.openfda?.package_ndc || undefined),
        ...
        console.log(`[fetchOpenFdaDataByDrugName] Transformed ${transformedResults.length} product-centric results from Drugs@FDA.`);

        return {
          results: transformedResults,
          total: rawDrugsfdaData.meta?.results?.total || transformedResults.length,
        };
      }
      
      console.warn(`[fetchOpenFdaDataByDrugName] No results in Drugs@FDA data for drug name: ${drugName}.`);
      return rejectWithValue(`No results in Drugs@FDA data for drug name: ${drugName}. Parsed data: ${JSON.stringify(rawDrugsfdaData)}`);
    } catch (error: any) {
      console.error(`[fetchOpenFdaDataByDrugName] General error for ${drugName} with Drugs@FDA:`, error);
      return rejectWithValue(error.message || `Failed to fetch data from Drugs@FDA for ${drugName}`);
    }
  }
);
...
export const fetchSplDetailFromDailyMed = createAsyncThunk<
  ...
  { rejectValue: { splSetId: string; error: string; dailyMedErrorBody?: string } } // Typed errors for rejection
>(
  'fdaData/fetchSplDetailFromDailyMed',
  async (splSetId, { rejectWithValue }) => {
    console.log(`[DEBUG fetchSplDetailFromDailyMed] START for SETID: ${splSetId}, URL: /api/dailymed/${splSetId}`);
    try {
      console.log(`[DEBUG fetchSplDetailFromDailyMed] Attempting fetch for ${splSetId} via proxy (expecting XML)`);
      const response = await fetch(`/api/dailymed/${splSetId}`); // Calls our Next.js API route
      
      console.log(`[DEBUG fetchSplDetailFromDailyMed] Fetch call completed for ${splSetId}. Response received.`);
      ...
      if (!response.ok) {
        ...
        let errorData = { error: `Unknown error from proxy for SETID ${splSetId}`, dailyMedErrorBody: responseText };
        try {
          ...
          if (parsedProxyError && parsedProxyError.error) {
            ...
            errorData.dailyMedErrorBody = parsedProxyError.dailyMedErrorBody || responseText;
          }
        } catch (e) {
          // If parsing fails, use the raw text as part of the error.
          console.warn(`[DEBUG fetchSplDetailFromDailyMed] Failed to parse error response from proxy as JSON for ${splSetId}. Raw text: ${responseText}`);
        }
        console.error(`[DEBUG fetchSplDetailFromDailyMed] response.ok is false for ${splSetId}. Error: ${errorData.error}. DailyMed Body: ${errorData.dailyMedErrorBody}`);
        return rejectWithValue({ splSetId, error: errorData.error, dailyMedErrorBody: errorData.dailyMedErrorBody });
      }

      // Check content type if needed, though proxy should ensure it sends XML for success
      const contentType = response.headers.get('content-type');
      if (!contentType || !contentType.includes('application/xml')) {
        console.warn(`[DEBUG fetchSplDetailFromDailyMed] Unexpected content type for ${splSetId}: ${contentType}. Body: ${responseText}`);
      ...
      console.log(`[DEBUG fetchSplDetailFromDailyMed] Successfully fetched XML for ${splSetId}. Length: ${responseText.length}`);
      return { spl_set_id: splSetId, xml_content: responseText };

    } catch (error: any) {
      console.error(`[DEBUG fetchSplDetailFromDailyMed] CATCH BLOCK for ${splSetId}. Error object:`, error);
      let errorMessage = 'Failed to fetch XML for DailyMed SPL';
      if (error.name && error.message) {
        errorMessage = `${error.name}: ${error.message}`;
      }
      console.error(`[fetchSplDetailFromDailyMed] Final error message for ${splSetId}: ${errorMessage}`);
...
>(
  ...
  async (drugName, { rejectWithValue }) => {
    if (!drugName || drugName.trim() === '') {
      console.error('[fetchSplsFromDailyMedByName] Error: No drug name provided.');
      return rejectWithValue('No drug name provided to fetch SPLs from DailyMed.');
    }
    const encodedDrugName = encodeURIComponent(drugName.toLowerCase());
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[fetchSplsFromDailyMedByName] Error details: ${errorMsg}`); // Log the detailed error
        if (response.status === 404) {
      ...
      if (apiResponse && apiResponse.data && Array.isArray(apiResponse.data)) {
        const transformedSpls: DailyMedSplDetail[] = apiResponse.data.map((item: any) => {
          ...
          if (item.dosage_form) {
            dosageForms = Array.isArray(item.dosage_form) ? item.dosage_form : [item.dosage_form];
          } else if (item.attributes?.dosage_form) { // Check nested attributes as another common pattern
             dosageForms = Array.isArray(item.attributes.dosage_form) ? item.attributes.dosage_form : [item.attributes.dosage_form];
        ...
        console.log(`[fetchSplsFromDailyMedByName] Fetched and transformed ${transformedSpls.length} SPLs from DailyMed for drug: ${drugName}`);
        return transformedSpls;
      } else {
        console.warn(`[fetchSplsFromDailyMedByName] No 'data' array in DailyMed response for ${drugName}, or response was not as expected. Response:`, apiResponse);
        return []; // Return empty array if no data or unexpected structure
      }
    } catch (error: any) {
      console.error(`[fetchSplsFromDailyMedByName] General error for ${drugName}:`, error);
      return rejectWithValue(error.message || `Failed to fetch SPLs for ${drugName} from DailyMed`);
    }
  }
);
...
const fdaDataSlice = createSlice({
  ...
  reducers: {
    ...
    setPrioritizedSpls: (state, action: PayloadAction<Record<string, ParsedSplProductData>>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchNdcsByRxcui.fulfilled, (state, action: PayloadAction<string[]>) => {
      ...
      .addCase(fetchFdaDataByNdcs.fulfilled, (state, action: PayloadAction<FetchFdaDataPayload>) => {
    ...
    .addCase(fetchOpenFdaDataByDrugName.pending, (state, action) => {
      console.log('[fdaDataSlice/fetchOpenFdaDataByDrugName.pending] Action:', action);
      state.status = 'loading';
      state.error = null;
    ...
    .addCase(fetchOpenFdaDataByDrugName.fulfilled, (state, action: PayloadAction<FetchFdaDataPayload>) => {
      console.log('[fdaDataSlice/fetchOpenFdaDataByDrugName.fulfilled] Action with transformed Drugs@FDA results:', action);
      ...
      const filteredOpenFdaResults = rawResults.filter(result => {
        if (result.openfda && result.openfda.application_type && Array.isArray(result.openfda.application_type)) {
          return result.openfda.application_type.some(appType => 
        ...
        if (result.application_number) {
          return result.application_number.startsWith('NDA') || 
      ...
      console.log(`[fdaDataSlice/fetchOpenFdaDataByDrugName.fulfilled] Filtered ${rawResults.length} Drugs@FDA results down to ${filteredOpenFdaResults.length} based on NDA/ANDA/BLA type.`);
    ...
    .addCase(fetchOpenFdaDataByDrugName.rejected, (state, action) => {
      console.error('[fdaDataSlice/fetchOpenFdaDataByDrugName.rejected] Action:', action);
      state.status = 'failed';
      state.error = action.payload;
      state.openFdaResults = [];
      state.totalOpenFdaResults = 0;
    ...
    .addCase(fetchSplDetailFromDailyMed.pending, (state, action) => {
      ...
      console.log(`[DailyMedReducer] fetchSplDetailFromDailyMed.pending for SETID: ${splSetId}`);
    ...
    .addCase(fetchSplDetailFromDailyMed.fulfilled, (state, action: PayloadAction<DailyMedSplDetail>) => {
      const { spl_set_id, xml_content } = action.payload;
      console.log(`[DailyMedReducer] fetchSplDetailFromDailyMed.fulfilled for SETID: ${spl_set_id}, XML length: ${xml_content?.length}`);
    ...
    .addCase(fetchSplDetailFromDailyMed.rejected, (state, action) => {
      const payload = action.payload as { splSetId: string; error: string; dailyMedErrorBody?: string };
      ...
      console.error(`[DailyMedReducer] fetchSplDetailFromDailyMed.rejected for SETID: ${splSetId}. Error: ${errorMessage}`, payload?.dailyMedErrorBody ? `DailyMed Body: ${payload.dailyMedErrorBody}` : '');
      state.dailyMedDetails[splSetId] = {
        ...state.dailyMedDetails[splSetId],
        status: 'failed',
        error: errorMessage,
    ...
    .addCase(fetchSplsFromDailyMedByName.fulfilled, (state, action: PayloadAction<DailyMedSplDetail[]>) => {
...
export const selectOpenFdaResults = (state: RootState) => state.fdaData.openFdaResults;
...
export const selectTotalOpenFdaResults = (state: RootState) => state.fdaData.totalOpenFdaResults;

// Selector for DailyMed details
export const selectDailyMedDetails = (state: RootState) => state.fdaData.dailyMedDetails;
export const selectPrioritizedSplsByDosageForm = (state: RootState) => state.fdaData.prioritizedSplsByDosageForm;

// Selectors for the new DailyMed SPL list fetched by drug name
export const selectDailyMedSplListForDrugName = (state: RootState) => state.fdaData.dailyMedSplListForDrugName;
export const selectDailyMedSplListStatus = (state: RootState) => state.fdaData.dailyMedSplListStatus;
export const selectDailyMedSplListError = (state: RootState) => state.fdaData.dailyMedSplListError;
export const selectCurrentDrugNameQuery = (state: RootState) => state.fdaData.currentDrugNameQuery;
