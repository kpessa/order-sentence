...
export async function POST(request: NextRequest) {
  try {
    ...
    const logStrings = (logs as LogEntry[]).map((logEntry) => {
      const paramsString =
        logEntry.optionalParams && logEntry.optionalParams.length > 0
          ? logEntry.optionalParams
    ...
    console.log(
      `[API/LOG] Received and saved ${logs.length} log entries to ${logFilePath}`
    );
    return NextResponse.json(
    ...
    return NextResponse.json(
      { message: 'Failed to process logs.', error: errorMessage },
      { status: 500 }
    );
  }
}
