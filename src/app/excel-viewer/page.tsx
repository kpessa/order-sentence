...
} from '@/lib/store/slices/excelDataSlice';
...
function ExcelViewerContent() {
  ...
  console.log(
    '[ExcelViewerContent] excelLoadingStatus from store:',
    excelLoadingStatus
  );
  console.log(
    '[ExcelViewerContent] excelData.length from store:',
    excelData.length
  );

  useEffect(() => {
    console.log(
      '[ExcelViewerContent] useEffect triggered. Current status:',
      excelLoadingStatus,
      'Data length:',
      excelData.length
    );
    if (
      selectedDrugFromStore &&
      (selectedDrugFromStore.rxcui !== rxcuiFromUrl ||
        selectedDrugFromStore.name !== drugNameFromUrl)
    ) {
      console.log(
        '[ExcelViewerContent] Drug selection in Redux changed. Updating URL.'
      );
  ...
  useEffect(() => {
    console.log(
      '[ExcelViewerContent] useEffect for ExcelData. Current excelLoadingStatus:',
      excelLoadingStatus,
      'ExcelData length:',
      excelData.length
    );
    if (
      excelData.length === 0 &&
      excelLoadingStatus !== 'succeeded' &&
      excelLoadingStatus !== 'failed'
    ) {
      dispatch(fetchExcelData());
    }
  }, [dispatch, excelLoadingStatus, excelData.length]);

  const isLoadingExcel =
    excelLoadingStatus === 'pending' ||
    (excelLoadingStatus === 'idle' && excelData.length === 0);
  ...
  if (excelError) {
    console.log(
      '[ExcelViewerContent] Rendering: Error loading Excel data.',
      excelError
    );
    return (
      <div className="text-center text-red-600">
        Error loading Excel data: {excelError}
      </div>
    );
  }
  ...
  if (!rxcuiFromUrl || !drugNameFromUrl) {
    console.log(
      '[ExcelViewerContent] Rendering: Drug information not provided in URL.'
    );
    return (
      <div className="text-center">
        ...
        <p className="mb-2 text-sm">
          Please select a drug using the search in the filter panel.
        </p>
  ...
  return (
    <div>
        ...
        {isEditingDrug ? (
          ...
          <div className="flex items-center mt-1">
            <p className="text-xl text-gray-600">
              For:{' '}
              <span className="font-semibold">{decodedDrugNameFromUrl}</span>{' '}
              (RxCUI: {rxcuiFromUrl})
            </p>
            <Button
              variant="ghost"
      ...
      <ExcelOrderSentenceTable
        rxcui={rxcuiFromUrl}
        drugName={decodedDrugNameFromUrl}
        excelData={excelData}
      />

      <div className="mt-8 text-sm text-gray-500">
        <p>
          This table displays order sentences from the centrally managed Excel
          file.
        </p>
      </div>
    </div>
