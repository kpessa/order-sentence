import { parseOrderSentence, ParsedOrderSentence } from '../parseOrderSentence'
...
describe('parseOrderSentence', () => {
  ...
  describe('frequency parsing', () => {
    it('should parse simple frequencies', () => {
      const frequencies = ['Daily', 'BID', 'TID', 'QID', 'One Time', 'Every 8 hours']
    ...
    it('should parse complex frequencies', () => {
      const result = parseOrderSentence('10 mg, Oral, Tab, Every 6 hours for 7 days')
      expect(result.FREQUENCY).toBe('Every 6 hours for 7 days')
    })

    it('should handle frequency with multiple commas', () => {
      const result = parseOrderSentence('10 mg, Oral, Tab, Take with food, daily')
  ...
  describe('PRN parsing', () => {
    ...
    it('should parse PRN with reason', () => {
      const result = parseOrderSentence('10 mg, Oral, Tab, Daily, PRN for pain')
      expect(result.FREQUENCY).toBe('Daily')
      expect(result.PRN).toBe('PRN')
    ...
    it('should parse PRN with reason after comma', () => {
      const result = parseOrderSentence('10 mg, Oral, Tab, Daily, PRN, for nausea')
      expect(result.FREQUENCY).toBe('Daily')
      expect(result.PRN).toBe('PRN')
    ...
    it('should handle PRN in middle of frequency', () => {
      const result = parseOrderSentence('10 mg, Oral, Tab, Every 4 hours PRN for pain')
    ...
    it('should handle PRN at beginning of frequency', () => {
      const result = parseOrderSentence('10 mg, Oral, Tab, PRN for breakthrough pain')
    ...
    it('should handle multiple PRN mentions (should find first)', () => {
      const result = parseOrderSentence('10 mg, Oral, Tab, PRN for pain, note: PRN only')
      expect(result.FREQUENCY).toBeUndefined()
      expect(result.PRN).toBe('PRN')
  ...
  describe('edge cases and complex scenarios', () => {
    ...
    it('should handle complex medication instruction', () => {
      const result = parseOrderSentence('Apply to affected area(s), Topical, Cream, Daily for 7 days')
  ...
  describe('real-world examples', () => {
    ...
    it('should parse IV medication with PRN (limitation: drug name first)', () => {
      const result = parseOrderSentence('Morphine 2 mg, IV, Inj, Every 4 hours, PRN for pain')
    ...
    it('should parse topical medication (limitation: drug name first)', () => {
      const result = parseOrderSentence('Hydrocortisone 1%, Topical, Cream, Apply BID')
      // Function expects dose first, not drug name
      expect(result.DOSE).toBeUndefined()
      expect(result.DOSE_UOM).toBeUndefined()
    ...
    it('should parse insulin with units (limitation: drug name first)', () => {
      const result = parseOrderSentence('Regular Insulin 10 units, SQ, Inj, AC & HS')
      // Function expects dose first, not drug name
      expect(result.DOSE).toBeUndefined()
      expect(result.DOSE_UOM).toBeUndefined()
    ...
    it('should parse one-time medication (limitation: drug name first)', () => {
      const result = parseOrderSentence('Acetaminophen 650 mg, Oral, Tab, One Time')
      // Function expects dose first, not drug name
      expect(result.DOSE).toBeUndefined()
      expect(result.DOSE_UOM).toBeUndefined()
  ...
  describe('return value structure', () => {
    it('should return ParsedOrderSentence with correct interface', () => {
      const result = parseOrderSentence('400 mg, Oral, Tab, Daily, PRN for pain')
