...
>('excelData/fetchData', async (_, { rejectWithValue }) => {
  console.log('[excelDataSlice] Starting fetch for:', FILE_PATH);
  try {
    const response = await fetch(FILE_PATH);
    if (!response.ok) {
      console.error(
        '[excelDataSlice] Failed to fetch Excel file:',
        response.statusText
      );
      return rejectWithValue(
        `Failed to fetch Excel file: ${response.statusText}`
      );
    }
    ...
    console.log(
      `[excelDataSlice] Parsing sheet data. Using data from row ${HEADER_ROW} (1-indexed) as headers.`
    );
    ...
    console.log(
      `[excelDataSlice] Successfully converted ${jsonData.length} rows to objects.`
    );
    if (jsonData.length > 0) {
      console.log(
        '[excelDataSlice] Sample of parsed data (first object raw):',
        jsonData[0]
      );
    }
    return jsonData;
...
const excelDataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchExcelData.fulfilled,
        (state, action: PayloadAction<ExcelRow[]>) => {
          state.loading = 'succeeded';
          state.data = action.payload;
          console.log(
            '[excelDataSlice] Fetching data: succeeded. Rows:',
      ...
      .addCase(fetchExcelData.rejected, (state, action) => {
        ...
        console.error(
          '[excelDataSlice] Fetching data: failed. Error:',
          state.error
        );
      });
  },
...
export const selectAllExcelData = (state: { excelData: ExcelDataState }) =>
  state.excelData.data;
export const selectExcelDataLoading = (state: { excelData: ExcelDataState }) =>
  state.excelData.loading;
export const selectExcelDataError = (state: { excelData: ExcelDataState }) =>
  state.excelData.error;
