...
describe('drugSearchSlice', () => {
  ...
  describe('reducers', () => {
    ...
    describe('setStatus', () => {
      ...
      it('should handle all status types', () => {
        const statuses = ['idle', 'loading', 'succeeded', 'failed', 'retrying'] as const
  ...
  describe('selectors', () => {
    ...
    it('should select query', () => {
      const query = selectDrugSearchQuery(store.getState())
      expect(query).toBe('test query')
    })

    it('should select results', () => {
      const results = selectDrugSearchResults(store.getState())
      expect(results).toEqual([])
    })

    it('should select selected drug', () => {
      const selectedDrug = selectSelectedDrug(store.getState())
      expect(selectedDrug).toEqual(mockSelectedDrug)
    })

    it('should select status', () => {
      const status = selectDrugSearchStatus(store.getState())
      expect(status).toBe('succeeded')
    })

    it('should select error', () => {
      const error = selectDrugSearchError(store.getState())
      expect(error).toBe(null)
    })

    it('should select retries', () => {
      const retries = selectDrugSearchRetries(store.getState())
  ...
  describe('fetchDrugResults async thunk', () => {
    ...
    it('should filter out non-ingredient drugs', async () => {
      ...
      expect(result.payload[0].tty).toBe('IN')
    })
    ...
    it('should handle non-200 API responses', async () => {
      ...
      expect(result.type).toBe('drugSearch/fetchResults/rejected')
      expect(result.payload).toBe('RxNorm API (approximateTerm) request failed with status: 500')
    })
    ...
    it('should sort results by score and name', async () => {
      ...
      expect(result.payload[0].name).toBe('Alpha Drug') // Higher score, alphabetically first
      expect(result.payload[1].name).toBe('Lisinopril') // Higher score, alphabetically second
      expect(result.payload[2].name).toBe('Zebra Drug') // Lower score
    })
