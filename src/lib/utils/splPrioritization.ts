...
const extractTextRecursively = (node: any): string => {
  ...
  for (const key in node) {
    if (key === '$' || key === 'caption' || key === 'footnote' || key === 'footnoteRef') continue; 
    
    if (Array.isArray(node[key])) {
      node[key].forEach((childNode: any) => text += extractTextRecursively(childNode));
 ...
 */
async function parseSplXml(splDetail: DailyMedSplDetail): Promise<ParsedSplProductData> {
  // Utility to stringify with depth limit to avoid huge logs - MOVED TO TOP OF FUNCTION
  const stringifyWithDepth = (obj: any, depth = 2) => { // Default depth is 2, can be overridden
    let currentDepth = 0;
    const cache = new Set();
  ...
  let published_date: string | { value?: string; [key: string]: any } | undefined = undefined;
  ...
  const processFormCodeNode = (formCodeNode: any, sourceLocation: string): string[] => {
    const extractedForms: string[] = [];
    console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - processFormCodeNode called from ${sourceLocation}. formCodeNode:`, JSON.stringify(formCodeNode, null, 2)); // Log the input
    if (!formCodeNode) return extractedForms;

    const itemsToProcess = Array.isArray(formCodeNode) ? formCodeNode : [formCodeNode];
    itemsToProcess.forEach(item => {
      if (item?.displayName) {
        const formName = typeof item.displayName === 'object' && item.displayName._ ? item.displayName._ : item.displayName;
        if (formName && typeof formName === 'string') {
          console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - Found dosage form '${formName}' from ${sourceLocation}`);
          extractedForms.push(formName.toUpperCase());
        } else {
          console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - Found displayName but not a string or empty from ${sourceLocation}. Value: ${JSON.stringify(formName)}`);
        }
      } else {
  ...
  if (!splDetail.xml_content) {
    console.warn(`[parseSplXml] SPL SET ID ${splDetail.spl_set_id} has no xml_content to parse.`);
    return {
      published_date,
      dosage_forms,
  ...
  try {
    ...
    if (parsedXml.document) {
      console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - Top-level keys of parsedXml.document:`, Object.keys(parsedXml.document).join(', '));
      // Attempt to find published_date (document level)
      if (parsedXml.document.effectiveTime) {
        published_date = parsedXml.document.effectiveTime.value || parsedXml.document.effectiveTime.$?.value || parsedXml.document.effectiveTime;
      }

      // Log keys of document.component if it exists
      if (parsedXml.document.component) {
        console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - Keys of parsedXml.document.component:`, Object.keys(parsedXml.document.component).join(', '));
      } else {
        console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - parsedXml.document.component NOT FOUND`);
      }

      // --- TARGETED DOSAGE FORM EXTRACTION --- 
      // Path 1: Directly under document.component.manufacturedProduct.manufacturedProduct.formCode
      if (parsedXml.document.component?.manufacturedProduct?.manufacturedProduct?.formCode) {
        console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - Checking path: document.component.manufacturedProduct.manufacturedProduct.formCode`);
        dosage_forms.push(...processFormCodeNode(parsedXml.document.component.manufacturedProduct.manufacturedProduct.formCode, 'document.component.manufacturedProduct.manufacturedProduct'));
      } else {
        console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - Path not found or formCode missing: document.component.manufacturedProduct.manufacturedProduct.formCode. Parent:`, parsedXml.document.component?.manufacturedProduct?.manufacturedProduct ? 'Exists' : 'Not Found');
      }
          ...
          if (comp?.section) {
            ...
            for (const section of sectionDetails) {
              if (!published_date && section.effectiveTime) { 
                published_date = section.effectiveTime.value || section.effectiveTime.$?.value || section.effectiveTime;
              }
              
              const currentSectionCode = section.$?.code;
              const currentSectionTitle = section.title?._ || section.title;
              const titleToLog = typeof currentSectionTitle === 'object' ? JSON.stringify(currentSectionTitle) : currentSectionTitle;
              ...
              if (section.$ && section.$.code === '34068-7') { 
                isDandASection = true;
              } else if (currentSectionTitle && typeof currentSectionTitle === 'string') {
                const lowerTitle = currentSectionTitle.toLowerCase();
                const normalizedTitle = lowerTitle.replace(/\s+/g, ' ').trim();
                if (normalizedTitle.includes('dosage and administration') || normalizedTitle.includes('directions')) {
                  isDandASection = true;
                }
              }
              if (isDandASection) {
                ...
                if(section.component && Array.isArray(section.component)) {
                  section.component.forEach((subComp: any) => {
                    if (subComp.section && subComp.section.text) {
                      extracted += extractTextRecursively(subComp.section.text) + '\n';
                    } else if (subComp.section) {
                      extracted += extractTextRecursively(subComp.section) + '\n';
                    }
                  });
                } else if (section.component && section.component.section) {
                  if (section.component.section.text) {
                    extracted += extractTextRecursively(section.component.section.text) + '\n';
                  } else {
                    extracted += extractTextRecursively(section.component.section) + '\n';
                  }
                }
                if (extracted.trim()) {
                  dosageAndAdministrationText = extracted.trim().replace(/\s+/g, ' ');
                }
              }

              // Dosage Form from section.subject.manufacturedProduct.manufacturedProduct.formCode
              if (section.subject?.manufacturedProduct?.manufacturedProduct?.formCode) {
                const fcNode = section.subject.manufacturedProduct.manufacturedProduct.formCode;
                console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - Checking path in section (Title: ${titleToLog || 'N/A'}): section.subject.manufacturedProduct.manufacturedProduct.formCode. Node found: ${fcNode ? 'Yes' : 'No'}`);
                if (fcNode) {
                  dosage_forms.push(...processFormCodeNode(fcNode, `section.subject.manufacturedProduct (section title: ${titleToLog || 'N/A'})`));
                }
              } else if (section.subject?.manufacturedProduct?.manufacturedProduct) {
              ...
              if (section.manufacturedProduct?.formCode) {
                 ...
                 console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - Checking path in section (Title: ${titleToLog || 'N/A'}): section.manufacturedProduct.formCode. Node found: ${fcNodeDirect ? 'Yes' : 'No'}`);
                 if (fcNodeDirect) {
                    dosage_forms.push(...processFormCodeNode(fcNodeDirect, `section.manufacturedProduct (section title: ${titleToLog || 'N/A'})`));
                 }
              }
            }
          }
      ...
      console.log(`[parseSplXml] SETID ${splDetail.spl_set_id} - parsedXml.document NOT FOUND.`);
    }

    const uniqueDosageForms = Array.from(new Set(dosage_forms.filter(df => df && typeof df === 'string')));
    ...
    if (typeof published_date === 'string') {
      ...
      } else if (published_date.$ && typeof published_date.$ === 'object' && published_date.$.value && typeof published_date.$.value === 'string') {
        dateToLog = published_date.$.value;
      } else {
    ...
    console.log(`[parseSplXml] Parsed SETID ${splDetail.spl_set_id}: Date: ${dateToLog || 'N/A'}, Forms: ${uniqueDosageForms.length > 0 ? uniqueDosageForms.join(', ') : 'N/A'}, D&A Found: ${dosageAndAdministrationText ? 'Yes' : 'No'}`);
    ...
    console.error(`[parseSplXml] Error parsing XML for SPL SET ID ${splDetail.spl_set_id}:`, error);
    return { 
...
export async function performSplPrioritization(
  dailyMedSplDetails: Record<string, { data?: DailyMedSplDetail; status: string; error?: string | null }>
): Promise<Record<string, ParsedSplProductData>> {
  console.log('[performSplPrioritization] Input SPL details object (summarized):', 
    Object.fromEntries(
  ...
  for (const setId in dailyMedSplDetails) {
    ...
    if (detailEntry.status === 'succeeded' && detailEntry.data && detailEntry.data.xml_content) {
      successfullyFetchedSpls.push(detailEntry.data);
    }
  }

  if (successfullyFetchedSpls.length === 0) {
    console.log('[performSplPrioritization] No successfully fetched SPLs with XML content to process.');
    return {};
  }

  console.log('[performSplPrioritization] SPLs with XML to parse (summarized):', 
  ...
  const parsedSplDataPromises: Promise<ParsedSplProductData>[] = successfullyFetchedSpls.map(parseSplXml);
  const parsedSplProductDataArray: ParsedSplProductData[] = await Promise.all(parsedSplDataPromises);

  console.log('[performSplPrioritization] All SPLs parsed (summarized):', 
    parsedSplProductDataArray.map(data => ({
      ...
      published_date: (typeof data.published_date === 'string' ? data.published_date :
                      (typeof data.published_date === 'object' && data.published_date !== null && data.published_date.value ? data.published_date.value :
                      (typeof data.published_date === 'object' && data.published_date !== null && data.published_date.$ && data.published_date.$.value ? data.published_date.$.value : JSON.stringify(data.published_date)))) || 'N/A',
      dosage_forms: data.dosage_forms.join(', ') || 'N/A',
      xml_content_summary: data.xml_content ? `XML (length: ${data.xml_content.length})` : 'No XML Content',
  ...
  parsedSplProductDataArray.forEach(parsedData => {
    ...
    console.log(`[performSplPrioritization] Grouping for SETID ${parsedData.spl_set_id}, Dosage Forms: ${JSON.stringify(parsedData.dosage_forms)}, D&A Found: ${parsedData.dosageAndAdministrationText ? 'Yes (length: ' + parsedData.dosageAndAdministrationText.length + ')' : 'No'}`);

    const formKey = (parsedData.dosage_forms && parsedData.dosage_forms.length > 0 && typeof parsedData.dosage_forms[0] === 'string') 
  ...
  console.log('[performSplPrioritization] Prioritizing SPLs within each dosage form group by date.');
  ...
  for (const formKey in groupedByDosageForm) {
    ...
    console.log(`[performSplPrioritization] SPLs in '${formKey}' (pre-sort):`, 
      splsInGroup.map(s => {
        let dateToLog: string | undefined = undefined;
        if (typeof s.published_date === 'string') {
          dateToLog = s.published_date;
        } else if (typeof s.published_date === 'object' && s.published_date !== null) {
          if (s.published_date.value && typeof s.published_date.value === 'string') {
            dateToLog = s.published_date.value;
          } else if (s.published_date.$ && typeof s.published_date.$ === 'object' && s.published_date.$.value && typeof s.published_date.$.value === 'string') {
            dateToLog = s.published_date.$.value;
          } else {
        ...
        return { 
          ...
          dosageAndAdministrationFound: s.dosageAndAdministrationText ? 'Yes' : 'No'
        };
      })
    );
    ...
    splsInGroup.sort((a, b) => {
      ...
      const getDateString = (dateField: string | { value?: string; [key: string]: any } | undefined): string | null => {
        if (typeof dateField === 'string') return dateField;
        if (typeof dateField === 'object' && dateField !== null) {
          if (dateField.value && typeof dateField.value === 'string') return dateField.value;
          if (dateField.$ && typeof dateField.$ === 'object' && dateField.$.value && typeof dateField.$.value === 'string') return dateField.$.value;
        }
        return null;
      };
    ...
    console.log(`[performSplPrioritization] SPLs in '${formKey}' (POST-sort by date - most recent first):`, 
      splsInGroup.map(s => {
        let dateToLog: string | undefined = undefined;
        if (typeof s.published_date === 'string') {
          dateToLog = s.published_date;
  ...
  console.log('[performSplPrioritization] Final prioritized SPLs by dosage form (summarized):', 
    Object.fromEntries(
      Object.entries(prioritized).map(([key, parsedDataVal]) => [
        key,
        {
          spl_set_id: parsedDataVal.spl_set_id,
          published_date: (typeof parsedDataVal.published_date === 'string' ? parsedDataVal.published_date :
                          (typeof parsedDataVal.published_date === 'object' && parsedDataVal.published_date !== null && parsedDataVal.published_date.value ? parsedDataVal.published_date.value :
                          (typeof parsedDataVal.published_date === 'object' && parsedDataVal.published_date !== null && parsedDataVal.published_date.$ && parsedDataVal.published_date.$.value ? parsedDataVal.published_date.$.value : JSON.stringify(parsedDataVal.published_date)))) || 'N/A',
          xml_content_summary: parsedDataVal.xml_content ? `XML (length: ${parsedDataVal.xml_content.length})` : 'No XML Content',
          dosageAndAdministrationFound: parsedDataVal.dosageAndAdministrationText ? `Yes (length: ${parsedDataVal.dosageAndAdministrationText.length})` : 'No',
