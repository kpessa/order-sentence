...
import {
  parseOrderSentence,
  ParsedOrderSentence,
} from '@/lib/utils/parseOrderSentence';
...
//         : cn(
//             'bg-transparent hover:bg-white/20 dark:hover:bg-slate-800/20',
//             baseTextColor,
//             'border-current opacity-75 hover:opacity-100'
//           ),
//       className
//     )}
//   >
...
const smartIncludesFilter: FilterFn<ExcelRowWithParsed> = (
  ...
  if (Array.isArray(filterValue)) {
    ...
    return filterValue.some(
      (pill) => String(pill).toLowerCase() === rowValueString
    );
  }
...
export function ExcelOrderSentenceTable({
  ...
  const [descriptionCombinationFilter, setDescriptionCombinationFilter] =
    useState<'all' | 'combination' | 'single'>('all');
  const [selectedDescriptionPill, setSelectedDescriptionPill] = useState<
    string | undefined
  >(undefined);
  const [descriptionSearchText, setDescriptionSearchText] =
    useState<string>('');
  const [selectedEncounterGroup, setSelectedEncounterGroup] = useState<
    string | undefined
  >(undefined);
  const [selectedSynonymPill, setSelectedSynonymPill] = useState<
    string | undefined
  >(undefined);
  const [synonymSearchText, setSynonymSearchText] = useState<string>('');
  const [selectedSynonymType, setSelectedSynonymType] = useState<
    string | undefined
  >(undefined);
  const [catalogTypeSearchText, setCatalogTypeSearchText] =
    useState<string>('');
  const [orderEntryFormatSearchText, setOrderEntryFormatSearchText] =
    useState<string>('');

  // --- PARSED Data Panel Filter States (NEW) ---
  const [panelSelectedRoute, setPanelSelectedRoute] = useState<
    string | undefined
  >(undefined);
  const [panelSelectedForm, setPanelSelectedForm] = useState<
    string | undefined
  >(undefined);
  ...
  console.log(
    `[ExcelTable] Initial Data: ${excelData.length} rows. Drug: ${drugName}`
  );
  ...
  const baseDataAfterDrugFilter = useMemo(() => {
    ...
    const filtered = excelData.filter((row: ExcelRow) =>
      String(row['Description'] || '')
        .toLowerCase()
        .includes(lowercasedDrugName)
    );
    console.log(
      `[ExcelTable] Step 1 (drugName filter '${drugName}'): ${filtered.length} rows`
    );
    return filtered;
  }, [excelData, drugName]);
  ...
  const dataAfterRawPanelFilters = useMemo(() => {
    ...
    if (descriptionCombinationFilter !== 'all') {
      result = result.filter((row: ExcelRow) => {
        ...
        if (descriptionCombinationFilter === 'combination')
          return isCombination;
        if (descriptionCombinationFilter === 'single') return !isCombination;
        return true;
      });
    }
    if (selectedDescriptionPill)
      result = result.filter(
        (row: ExcelRow) =>
          String(row['Description'] || '') === selectedDescriptionPill
      );
    ...
    if (selectedEncounterGroup)
      result = result.filter(
        (row: ExcelRow) =>
          String(row['Encounter Group'] || '') === selectedEncounterGroup
      );
    if (selectedSynonymPill)
      result = result.filter(
        (row: ExcelRow) => String(row['Synonym'] || '') === selectedSynonymPill
      );
    ...
    if (selectedSynonymType)
      result = result.filter(
        (row: ExcelRow) =>
          String(row['Synonym Type'] || '') === selectedSynonymType
      );
    if (catalogTypeSearchText.trim() !== '') {
      const lower = catalogTypeSearchText.toLowerCase();
      result = result.filter((row: ExcelRow) =>
        String(row['Catalog Type'] || '')
          .toLowerCase()
          .includes(lower)
      );
    }
    if (orderEntryFormatSearchText.trim() !== '') {
      ...
      result = result.filter((row: ExcelRow) =>
        String(row['Order Entry Format'] || '')
          .toLowerCase()
          .includes(lower)
      );
    }
    console.log(
      `[ExcelTable] Step 2 (raw panel filters): ${result.length} rows`
    );
    return result;
  }, [
  ...
  const baseParsedDataForMode = useMemo(() => {
    if (displayMode === 'raw') {
      console.log(
        `[ExcelTable] Step 3 (raw mode, no parsing): ${dataAfterRawPanelFilters.length} rows`
      );
    ...
    console.log(
      `[ExcelTable] Step 3 (parsed mode, after parsing): ${parsed.length} rows`
    );
    return parsed;
  ...
  const dataAfterParsedPanelFilters = useMemo(() => {
    ...
    if (panelSelectedRoute) {
      result = result.filter(
        (row) => row._parsedSentence?.RXROUTE === panelSelectedRoute
      );
    }
    if (panelSelectedForm) {
      result = result.filter(
        (row) => row._parsedSentence?.DOSE_FORM === panelSelectedForm
      );
    }
    console.log(
      `[ExcelTable] Step 4 (parsed panel filters): ${result.length} rows`
    );
    return result;
  }, [
    baseParsedDataForMode,
    displayMode,
    panelSelectedRoute,
    panelSelectedForm,
  ]);
  ...
  const uniqueEncounterGroups = useMemo(
    () =>
      Array.from(
        new Set(
          baseDataAfterDrugFilter
            .map((row) => String(row['Encounter Group'] || '').trim())
            .filter(Boolean)
        )
      ).sort(),
    [baseDataAfterDrugFilter]
  );
  const uniqueSynonymTypes = useMemo(
    () =>
      Array.from(
        new Set(
          baseDataAfterDrugFilter
            .map((row) => String(row['Synonym Type'] || '').trim())
            .filter(Boolean)
        )
      ).sort(),
    [baseDataAfterDrugFilter]
  );
  const top5UniqueDescriptions = useMemo(
    () =>
      Array.from(
        new Set(
          baseDataAfterDrugFilter
  ...
  const top5UniqueSynonyms = useMemo(
    () =>
      Array.from(
        new Set(
          baseDataAfterDrugFilter
            .map((row) => String(row['Synonym'] || '').trim())
            .filter(Boolean)
        )
      )
        .sort()
  ...
  const uniquePanelRoutes = useMemo(() => {
    ...
    const dataForPanelOptions = dataAfterRawPanelFilters.map((row) =>
      parseOrderSentence(row['Sentence'] as string)
    );
    return Array.from(
      new Set(
        dataForPanelOptions.map((ps) => ps?.RXROUTE).filter(Boolean) as string[]
      )
    ).sort();
  }, [dataAfterRawPanelFilters, displayMode]);
  ...
  const uniqueEncounterGroupsForPopover = useMemo(
    () => getUniqueColumnValues('Encounter Group'),
    [getUniqueColumnValues]
  );
  const uniqueDescriptionsForPopover = useMemo(
    () => getUniqueColumnValues('Description'),
    [getUniqueColumnValues]
  );
  const uniqueSynonymsForPopover = useMemo(
    () => getUniqueColumnValues('Synonym'),
    [getUniqueColumnValues]
  );
  const uniqueSynonymTypesForPopover = useMemo(
    () => getUniqueColumnValues('Synonym Type'),
    [getUniqueColumnValues]
  );
  const uniqueOrderEntryFormatsForPopover = useMemo(
    () => getUniqueColumnValues('Order Entry Format'),
    [getUniqueColumnValues]
  );

  const uniqueParsedDoseForPopover = useMemo(
    () => getUniqueColumnValues('_parsedSentence.DOSE'),
    [getUniqueColumnValues]
  );
  const uniqueParsedUOMForPopover = useMemo(
    () => getUniqueColumnValues('_parsedSentence.DOSE_UOM'),
    [getUniqueColumnValues]
  );
  const uniqueParsedRouteForPopover = useMemo(
    () => getUniqueColumnValues('_parsedSentence.RXROUTE'),
    [getUniqueColumnValues]
  );
  const uniqueParsedFormForPopover = useMemo(
    () => getUniqueColumnValues('_parsedSentence.DOSE_FORM'),
    [getUniqueColumnValues]
  );
  const uniqueParsedFrequencyForPopover = useMemo(
    () => getUniqueColumnValues('_parsedSentence.FREQUENCY'),
    [getUniqueColumnValues]
  );
  const uniqueParsedPrnForPopover = useMemo(
    () => getUniqueColumnValues('_parsedSentence.PRN'),
    [getUniqueColumnValues]
  );
  const uniqueParsedPrnReasonForPopover = useMemo(
    () => getUniqueColumnValues('_parsedSentence.PRN_REASON'),
    [getUniqueColumnValues]
  );
  ...
  const rawColumns = useMemo<ColumnDef<ExcelRowWithParsed>[]>(
    () => [
      {
        ...
        cell: ({ getValue }) => (
          <div
            className={cn(
              'px-2 py-1 h-full',
              getColumnStyles('Catalog Type').cellBg,
              getColumnStyles('Catalog Type').text
            )}
          >
            {String(getValue())}
          </div>
        ),
        filterFn: smartIncludesFilter,
        enableColumnFilter: true,
      },
      {
        accessorKey: 'Encounter Group',
        header: ({ column }) =>
          createHeader(
            'Encounter Group',
            column,
            uniqueEncounterGroupsForPopover
          ),
        cell: ({ getValue }) => (
          <div
            className={cn(
              'px-2 py-1 h-full',
              getColumnStyles('Encounter Group').cellBg,
              getColumnStyles('Encounter Group').text
            )}
          >
            {String(getValue())}
          </div>
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <div
            className={cn(
              'px-2 py-1 h-full',
              getColumnStyles('Description').cellBg,
              getColumnStyles('Description').text
            )}
          >
            {String(getValue())}
          </div>
      ...
      },
      {
        accessorKey: 'Synonym',
        header: ({ column }) =>
          createHeader('Synonym', column, uniqueSynonymsForPopover),
        cell: ({ getValue }) => (
          <div
            className={cn(
              'px-2 py-1 h-full',
              getColumnStyles('Synonym').cellBg,
              getColumnStyles('Synonym').text
            )}
          >
            {String(getValue())}
          </div>
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <div
            className={cn(
              'px-2 py-1 h-full',
              getColumnStyles('Synonym Type').cellBg,
              getColumnStyles('Synonym Type').text
            )}
          >
            {String(getValue())}
          </div>
      ...
      {
        accessorKey: 'Order Entry Format',
        header: ({ column }) =>
          createHeader(
            'Order Entry Format',
            column,
            uniqueOrderEntryFormatsForPopover
          ),
        cell: ({ getValue }) => (
          <div
            className={cn(
              'px-2 py-1 h-full',
              getColumnStyles('Order Entry Format').cellBg,
              getColumnStyles('Order Entry Format').text
            )}
          >
            {String(getValue())}
          </div>
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <div
            className={cn(
              'px-2 py-1 h-full text-sm',
              getColumnStyles('Sentence').cellBg,
              getColumnStyles('Sentence').text
            )}
          >
            {String(getValue())}
          </div>
    ...
    [
      ...
      uniqueOrderEntryFormatsForPopover,
      descriptionCombinationFilter,
      setDescriptionCombinationFilter,
    ]
  );
  ...
  const parsedColumns = useMemo<ColumnDef<ExcelRowWithParsed>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (info) => (
          <div
            className={cn(
              'px-2 py-1 h-full',
              getColumnStyles('Default').cellBg,
              getColumnStyles('Default').text
            )}
          >
      ...
      {
        accessorKey: '_parsedSentence.FREQUENCY',
        header: ({ column }) =>
          createHeader(
            'Frequency',
            column,
            uniqueParsedFrequencyForPopover,
            'Default'
          ),
        cell: (info) => (
          <div
      ...
      {
        accessorKey: '_parsedSentence.PRN_REASON',
        header: ({ column }) =>
          createHeader(
            'PRN Reason',
            column,
            uniqueParsedPrnReasonForPopover,
            'Default'
          ),
        cell: (info) => (
          <div
      ...
      },
      {
        ...
        cell: (info) => (
          <div
            className={cn(
              'px-2 py-1 h-full text-xs text-gray-500 max-w-xs truncate',
              getColumnStyles('Sentence').cellBg,
              getColumnStyles('Sentence').text
            )}
  ...
  // Create the header renderer function outside of memoization
  const createHeader = (
    title: string,
    column: any,
    uniquePills?: string[],
    ...
    return (
      <div
        className={cn(
          'w-full h-full px-2 py-1.5 flex items-center justify-between',
          styles.bg,
          styles.text
        )}
      >
        <div
          onClick={(e: React.MouseEvent) => {
            ...
            console.log(
              `[ExcelTable] Header Clicked: ${column.id}, Shift: ${isShiftPressed}, Current Sort: ${column.getIsSorted()}, Current Index: ${column.getSortIndex()}`
            );
        ...
        <ColumnHeaderFilterPopover
          ...
          descriptionCombinationFilter={
            isDescriptionCol ? descriptionCombinationFilter : undefined
          }
          onDescriptionCombinationFilterChange={
            isDescriptionCol ? setDescriptionCombinationFilter : undefined
          }
        />
      </div>
    );
  ...
  useEffect(() => {
    console.log(
      '[ExcelTable] Sorting state changed:',
  ...
  useEffect(() => {
    console.log(
      `[ExcelTable] Table row model updated. Final visible rows: ${table.getRowModel().rows.length}`
    );
  }, [table]);
  ...
  const noResultsAfterAllFilters =
    excelData.length > 0 && table.getRowModel().rows.length === 0; // Check final rows after all TanStack filters
  ...
  const currentNoResultsMessage = () => {
    ...
    if (noResultsAfterAllFilters) {
      return (
        <p>
          No results match the current combined filter criteria (including
          column filters).
        </p>
      );
    }
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          <div className="flex items-center space-x-2">
            <Switch
              ...
              className=""
            />
            <Label htmlFor="displayModeToggle" className="text-sm">
              Show Parsed
            </Label>
          </div>
        </div>
        ...
        <>
                ...
                {table.getRowModel().rows.map((row) => (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <td
                        key={cell.id}
                        className="whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 p-0"
                      >
                        {flexRender(
          ...
          <div className="mt-4 flex items-center justify-between flex-wrap gap-2">
            <div className="text-sm text-gray-700 dark:text-gray-300">
              Showing {table.getRowModel().rows.length} of {dataForTable.length}{' '}
              rows (filtered from {excelData.length} total after drug
              selection). Page{' '}
              <strong>
